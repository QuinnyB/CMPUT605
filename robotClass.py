'''
Class definition for Mini Bento robot arm controller (https://github.com/BLINCdev/Mini-Bento)
Allows reading from a motor, setting goal position of a given motor, moving a specified amount, stopping a motor,
...
class MiniBento:
    ...
    def stop_motor(self, motor_id):
        # Stop motor by setting current position as new goal position
        with self.lock:     # Wait our turn to use the COM port
            p, _, _ = self.packetHandler.read4ByteTxRx(self.portHandler, motor_id, self.addr_present_position)
            if p is not None:
                self.packetHandler.write4ByteTxRx(self.portHandler, motor_id, self.addr_goal_position, p)
